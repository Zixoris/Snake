...
const score = {
     ...
     */
    init() {
        this.countEl = document.getElementById('scoreCount');
        this.drop();
    },
...
const map = {
    ...
    render(snakePointsArray, foodPoint) {
        ...
        this.usedCells.push(foodCell); //в данной строке добавляся foodPoint вместо foodCell
        //Вместо добавления ячейки передавался объект с координатами..
    },
};
 ...
 * @property {int} maxX Максимальное положение змейки по оси Х
 * @property {int} maxY максимальное положение змейки по оси Y
 */
const snake = {
    body: null,
    direction: null,
     ...
     * @param {int} rowsCount - Количество строк в поле
     * @param {int} colsCount - количество столбцов в поле
     */
    init(startBody, direction, rowsCount, colsCount) {
        this.body = startBody;
        this.direction = direction;
        this.lastStepDirection = direction;
        this.maxX = colsCount - 1;
    ...
    /**
     * Возвращает максимально возможное положение по оси Y
     */
    getMaxRowsCount() {
        return this.maxY;
    },
    ...
    /**
     * Возвращает максимально возможное положение по оси X
     */
    getMaxColsCount() {
        return this.maxX;
    },
    ...
    getNextStepHeadPoint() {
        ...
        let xPoint = this.body[0].x,
            yPoint = this.body[0].y;
        // вычисляем точку, где окажется голова змейки в зависимости от направления.
        ...
        let maxRowsCount = this.getMaxRowsCount(),
            maxColsCount = this.getMaxColsCount()
        // Если змейка вышла за рамки, то телепортируем ее на другой конец поля
        if (xPoint < 0) {
            xPoint = maxColsCount;
        } else if (xPoint > maxColsCount) {
            xPoint = 0;
        }
        if (yPoint < 0) {
            yPoint = maxRowsCount;
        } else if (yPoint > maxRowsCount) {
            yPoint = 0;
        }
        return {
            x: xPoint,
